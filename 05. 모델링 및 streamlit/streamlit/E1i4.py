...
all_data = pd.read_csv(
    '/Users/leegahee/git_folder/04_프로젝트/2.주간프로젝트/마지막/진짜마지막/final_data_set_v5_링크수정.csv')

all_data.rename({'Longitude': 'longitude',
                 'Latitude': 'latitude'}, axis=1, inplace=True)
lda = pd.read_csv(
    '/Users/leegahee/git_folder/04_프로젝트/2.주간프로젝트/마지막/진짜마지막/LDA_tour_DB.csv', index_col=0)

food_ratio = pd.read_csv(
    '/Users/leegahee/git_folder/04_프로젝트/2.주간프로젝트/마지막/진짜마지막/food_ratio_result.csv', index_col=0)
tour_ratio = pd.read_csv(
    '/Users/leegahee/git_folder/04_프로젝트/2.주간프로젝트/마지막/진짜마지막/tour_ratio_result.csv', index_col=0)
...
def page6():
    ...
    hash_columns = [f'{hashtag} 비율' for hashtag in hashtags]

    if not pivot_table.empty:
        st.write("어떤 기준으로 보고 싶으세요? 기준별 인기순으로 나열합니다")

        # 해시태그 비율 버튼 생성
        columns = st.columns(len(hash_columns) + 2)  # 추가 버튼을 위해 열을 더 생성
        for i, hash_column in enumerate(hash_columns):
            if columns[i].button(f'{hash_column} 별로 보자!', key=f'button_{i}_{hash_column}'):
        ...
        # 합계/총합계 버튼 추가
        if columns[len(hash_columns)].button('방문자 수대로 보자!', key='button_sum'):
            sorted_pivot_table = pivot_table.sort_values(
                by='합계', ascending=False)
            st.session_state['sorted_pivot_table'] = sorted_pivot_table
            st.experimental_rerun()
